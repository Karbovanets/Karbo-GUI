...
Crypto::SecretKey WalletGreenWorker::getTransactionSecretKey(Crypto::Hash& _transactionId) const {
  Q_ASSERT(!m_wallet.isNull());
  SemaphoreLocker locker(m_walletSemaphore);
  Crypto::SecretKey txKey;
  m_dispatcher->remoteSpawn([this, &_transactionId, &txKey]() {
    SemaphoreUnlocker unlocker(m_walletSemaphore);
    txKey = m_wallet->getTransactionSecretKey(_transactionId);
  });
...
QString WalletGreenWorker::getTransactionProof(Crypto::Hash& _txid, CryptoNote::AccountPublicAddress& _address) const {
  Q_ASSERT(!m_wallet.isNull());
  SemaphoreLocker locker(m_walletSemaphore);
  std::string tx_proof;
  bool r;
  ...
  locker.wait();
  return r ? QString::fromStdString(tx_proof) : "";
}
...
QString WalletGreenWorker::getBalanceProof(quint64& _amount, QString& _message) const {
  ...
  SemaphoreLocker locker(m_walletSemaphore);
  std::string proof;
  m_dispatcher->remoteSpawn([this, _amount, _message, &proof]() {
    SemaphoreUnlocker unlocker(m_walletSemaphore);
    proof = m_wallet->getReserveProof(_amount, m_wallet->getAddress(0), _message.toStdString());
  });
...
bool WalletGreenWorker::verifyMessage(const QString &data, const QString &address, const QString &signature) const {
  return m_wallet->verifyMessage(data.toStdString(), address.toStdString(), signature.toStdString());
}
